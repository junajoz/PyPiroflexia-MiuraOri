...
#from GlobalK_edu_ver2 import GlobalK_edu_ver2
...
def PathAnalysis(truss, angles, F, b_lambda, MaxIcr):
    ...
    while icrm < MaxIcr:
        ...
        while err > tol and itera < Maxitera:
            ...
            # if np.allclose(IF, IF2):
            # print('IF is the same')
            # else:
            #    print('IF is different')
            # if np.allclose(K.A, K2.A):
            #    print('K is the same')
            # else:
            ...
            MUL[FreeDofs, :] = solve(K[np.ix_(FreeDofs, FreeDofs)].toarray(), MRS[FreeDofs, :])
            dUp = MUL[:, 0]
            dUr = MUL[:, 1]
            if itera == 1:
